#!/usr/bin/perl -w
######################################################################
#FileName:logTool_perMin_CSV.pl
...
}
{
    ...
    printf(OUT_FILE "\n開始時間\t重大なエラー\t通常なエラー\t接続数(総計)\t");
    printf(OUT_FILE "ACCEPTあり、CLOSEDあり数\tACCEPTあり、CLOSEDなし数\tACCEPTなし、CLOSEDあり数\tACCEPTなし、CLOSEDなし数\t");
    printf(OUT_FILE "SRCH数\tMOD数\tADD数\tDEL数\tPASSMOD数\t");
    printf(OUT_FILE "異常可能な接続(総計)\tACCEPTあり、後は何も操作がなし\tBINDあり、RESULTがなし\tSRCHあり、RESULTなし\t");
    printf(OUT_FILE "ADDあり、RESULTなし\tMODあり、RESULTなし\tDELあり、RESULTなし\tPASSMODあり、RESULTなし\tUNBINDあり、CLOSEDなし\t");
}
#行処理開始
print "処理中。。。\n";
...
#    printf(OUT_FILE "ACCEPTあり、CLOSEDあり数：%s%6d\n","\t",$close_con);
#    printf(OUT_FILE "ACCEPTあり、CLOSEDなし数：%s%6d\n","\t",$start_con);
#    printf(OUT_FILE "ACCEPTなし、CLOSEDあり数：%s%6d\n","\t",$noS_yesC);
#    printf(OUT_FILE "ACCEPTなし、CLOSEDなし数：%s%6d\n","\t",$noS_noC);
...
#    }
#	printf(OUT_FILE "%s\n","="x80);
    printf(OUT_FILE "\n%s\t%s\t%s\t%s\t",$start_time,$KeyCount,$all_err_count,$all_con);
	printf(OUT_FILE "%s\t%s\t%s\t%s\t",$close_con,$start_con,$noS_yesC,$noS_noC);
	printf(OUT_FILE "%s\t%s\t%s\t%s\t%s\t",$search_count,$mod_count,$add_count,$del_count,$pwdmod_count);
	my $count_tmp = $accept_only+$close_no+$bind_no_result+$srch_no_result+
	                $add_no_result+$mod_no_result+$del_no_result+$pwmod_no_result;
	printf(OUT_FILE "%s\t%s\t%s\t%s\t%s\t",$count_tmp,$accept_only,$bind_no_result,$srch_no_result,$add_no_result);
    printf(OUT_FILE "%s\t%s\t%s\t%s\t",$mod_no_result,$del_no_result,$pwmod_no_result,$close_no);
}
...
sub outputopen() {
	...
	$t_outFile = "$TOOL_DIR/Result_$str_time.csv";
	print "結果出力ファイル：\n  $t_outFile\n";
