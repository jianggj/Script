...
if(@ARGV == 1)
{
	...
	(my $control,$LOG_FILE)=@ARGV;
	if( uc($control) eq "A")
	{
		$ALL_MSG_OUT_PUT = "YES";
	}else{
		die "オプション指定エラー！\n".
				"詳細内容を出力したい場合、下記格式をご使用ください：\n".
				"  logAnalyze.pl  A  logfile\n".
				"**********Programe EXIT**********\n";
	}
}elsif(@ARGV == 0){
	die "ログファイルをご指定ください！\n**********Programe EXIT**********\n";
	...
	if($ALL_MSG_OUT_PUT eq "YES")
	{
		...
		if($close_no >0)
		{
			...
			foreach my $con_tmp (sort{$a<=>$b} keys %h_close_no)
			{
				...
				my $opcode_tmp = $hConnClass{$con_tmp}{"UNBIND_OP_NO"};
