...
{	
	...
	if($splitLine =~ /(?:.*)conn=(\d+)(?:.*)/i)
	{
		...
		{
	      $hConns{$con_code}=1;
		  $hConn_no_op{$con_code} = 1;
		  $hConn_ops{$con_code}{"op"}="";
		}
		#OP取得
	    if($splitLine =~/\bop=(\d+) (\S+).*/i)
	    {
			$op_code=$1;
			delete $hConn_no_op{$con_code};
			if($hConn_ops{$con_code}{"op"} eq "")
			{
			  $hConn_ops{$con_code}{"op"} = "".$op_code;
			}else{
			  unless($hConn_ops{$con_code}{"op"} =~ /\b$op_code\b/i)
			  {
			    $hConn_ops{$con_code}{"op"} .= "|".$op_code;
			  }
			}
...
#------------------------------後処理 --------------------------------
my $no_op_count = keys %hConn_no_op;
my $Conn_count = keys %hConns;
printf(OUT_FILE "\n%s%s%s\n","="x34,"統計結果一覧","="x34);
{
...
printf(OUT_FILE "\n%s%s%s\n","="x34,"統計結果詳細","="x34);
{
  ...
  {
    my @ops = split /\|/ ,$hConn_ops{$c_code}{"op"};

    unless(exists($hConn_no_op{$c_code}))
    {
	  printf(OUT_FILE "【接続番号：%d】OP番号数：%6d\n",$c_code,$#ops+1);
    }else{
...
sub outputopen() {
	...
	$t_outFile = "$TOOL_DIR/ConnS_OP_Count_Result_$str_time.log";
	print "結果出力ファイル：\n  $t_outFile\n";
